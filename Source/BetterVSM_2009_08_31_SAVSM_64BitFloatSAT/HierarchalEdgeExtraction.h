...
class HierarchalEdgeExtraction:public BasicSSMAlgorithm
{
public:
...
HierarchalEdgeExtraction::HierarchalEdgeExtraction()
{
	m_pEffect = NULL;
	m_pMaxLayout = NULL;
...
HRESULT HierarchalEdgeExtraction::OnD3D10CreateDevice(ID3D10Device *pDev10, const DXGI_SURFACE_DESC *pBackBufferSurfaceDesc, void *pUserContext)
{
	HRESULT hr;
	// create effect
    ...
    WCHAR str[MAX_PATH];
    V_RETURN(DXUTFindDXSDKMediaFileCch(str, MAX_PATH, (HIERARCHAL_EDGE_EXTRACTION_EFFECT_FILE_NAME) ));
    ID3D10Blob *pErrors;
    if (D3DX10CreateEffectFromFile(str, NULL, NULL, "fx_4_0", /*D3D10_SHADER_DEBUG|D3D10_SHADER_SKIP_OPTIMIZATION*/0, 0, pDev10, NULL, NULL, &m_pEffect, &pErrors, &hr) != S_OK)
    {
...
void	HierarchalEdgeExtraction::OnD3D10SwapChainReleasing( void* pUserContext )
{
...
HRESULT HierarchalEdgeExtraction::OnD3D10SwapChainResized( ID3D10Device* pDev10, IDXGISwapChain *pSwapChain, const DXGI_SURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext )
{
...
void HierarchalEdgeExtraction::OnD3D10FrameRender(CDXUTDialog &g_SampleUI,S3UTMesh &g_MeshScene,float g_fFilterSize,
									  ...
									  ID3D10Device* pDev10, double fTime, float fElapsedTime, void* pUserContext)
{
...

    {
        ...
        V(m_pEffect->GetVariableByName("g_vLightFlux")->AsVector()->SetRawValue(&vTmp, 0, sizeof(D3DXVECTOR4)));
...
void HierarchalEdgeExtraction::OnD3D10DestroyDevice( void* pUserContext )
{
