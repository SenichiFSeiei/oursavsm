...
// 
// The render algorithm of Mip mapped variance Shadow Maps
//
//----------------------------------------------------------------------------------
...
class MipVSM:public BasicSSMAlgorithm
{
public:
...
HRESULT MipVSM::CreateShader(ID3D10Device *pDev10)
{
	HRESULT hr;

	//Effect Creation
    WCHAR str[MAX_PATH];
    V_RETURN(DXUTFindDXSDKMediaFileCch(str, MAX_PATH, (MIP_VSM_EFFECT_FILE_NAME) ));
    ID3D10Blob *pErrors;
...
HRESULT MipVSM::OnD3D10CreateDevice(ID3D10Device *pDev10, const DXGI_SURFACE_DESC *pBackBufferSurfaceDesc, void *pUserContext)
{
	HRESULT hr;
...
void MipVSM::OnD3D10SwapChainReleasing( void* pUserContext )
{
    if( m_pShadowResult )
	    m_pShadowResult->OnD3D10SwapChainReleasing(pUserContext);
}
HRESULT MipVSM::OnD3D10SwapChainResized( ID3D10Device* pDev10, IDXGISwapChain *pSwapChain, const DXGI_SURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext )
{
...
void MipVSM::OnD3D10FrameRender(bool render_ogre, 
											  bool render_scene, 
											  CDXUTDialog &g_SampleUI,
											  S3UTMesh &g_MeshScene,
											  float g_fFilterSize,
...
void MipVSM::OnD3D10DestroyDevice( void* pUserContext )
{
