...
#else
Texture2D<float2> SatSrcTex;
Texture2D<float2> SatTex;
#endif
...
{
    if( abs(vPos.y) <= 0.05 ) vPos.y = -1;//ignore floor when rendering shadow map, this is a dirty trick which effectively avoid depth bias when rendering front face in shadow map
...
#else
	float2 ret = sum_result;
	return float4( ret.x, ret.y, 0, 0 );
#endif
}
