...
cbuffer cb1 : register(b1)
{
	...
	MS;
	float2 AroundQuad[4] = {{0,0},{0,1},{1,1},{1,0}};
};
...
float4 AccurateShadow(float3 vLightPos, float4 vDiffColor)
{
	...
	uint idx = 0;
	idx |= !(vDepthMin[3]>pixel_unit_z);
	idx<<=1;
	idx |=! (vDepthMin[2]>pixel_unit_z);
	idx<<=1;
	idx |=! (vDepthMin[1]>pixel_unit_z);
	idx<<=1;
	idx |=! (vDepthMin[0]>pixel_unit_z);
	
	float init_vf = 1;
	float frc_sum = frc_x + frc_y;
	if( 7 == idx && frc_sum > 1 )
		init_vf = 0;
	if( 13 == idx && frc_sum < 1 )
		init_vf = 0;
	if( 11 == idx && frc_y>frc_x )
		init_vf = 0;
	if( 14 == idx && frc_y<frc_x )
		init_vf = 0;
	if( 15 == idx )
		init_vf = 0;
	...
	[branch]if( nBLeft > nBRight || nBBottom > nBTop )
	{
		nBLeft  = refPX - 1;
		nBRight = refPX + 1;
		nBBottom = refPY - 1;
	...
	uint i = nBBottom;
	[loop]for ( uint j=nBLeft;j<=nBRight+1;++j)
	{
		...
		[branch]if( idx == 9 || idx == 11 || idx == 13 ){
			...
			while( vMaxCr.y <= nBTop + 1  && vMaxCr.y > nBBottom - 1 &&
			       vMaxCr.x <= nBRight + 1 && vMaxCr.x > nBLeft - 1    )
			{				
				[loop]for( uint cnt = 0; cnt<4; ++cnt )
				{
					iPixel = vMaxCr + AroundQuad[cnt];
					...
					float2 mar_sqr_idx = g_fMarchingSquare[idx];
					vMinCr = uint2(CoordX[ mar_sqr_idx.x ],CoordY[ mar_sqr_idx.x ]);
					
					//vMinCr == vMaxCr is very useful, it mask off those case
					//1: the current quad generating the current edge
					//2: cases the new edge does not start from the end of the old edge
					[branch]if( idx>1 && idx<15)
					//[branch]if( mar_sqr_idx.x != mar_sqr_idx.y && vMinCr == vMaxCr)
					{
						vMaxCr = uint2(CoordX[ mar_sqr_idx.y ],CoordY[ mar_sqr_idx.y ]);
						vMin = BackProj0(vDepthMin[mar_sqr_idx.x].xx,vLightPos,vMinCr);
						vMax = BackProj0(vDepthMin[mar_sqr_idx.y].xx,vLightPos,vMaxCr);
						int R = 32 - 1;
						vMin = floor((vMin*2*R + 1)/2);
						vMax = floor((vMax*2*R + 1)/2);
						int2 texcrd;
	...
	[loop]for( uint i = nBBottom; i<=nBTop+1;++i)
	{
		[loop]for ( uint j=nBLeft;j<=nBRight+1;++j)
		{
			uint2 iPixel = uint2(j,i);
			...
			float2 mar_sqr_idx = g_fMarchingSquare[idx];
			[branch]if( mar_sqr_idx.x != mar_sqr_idx.y ){
