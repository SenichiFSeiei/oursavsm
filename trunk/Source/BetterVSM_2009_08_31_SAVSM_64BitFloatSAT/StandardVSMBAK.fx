...
//calculate the filter kernel -------------------------------------------------------------------------------------
	float  scale = ( w - fLightZn )/w;
	float  LightWidthPers  = fFilterSize  * scale,
		   LightHeightPers = fFilterSize * scale;
	float4x4 g_mLightProj = mLightProj;
	float	 NpWidth  = fLightZn/g_mLightProj[0][0],
		     NpHeight = fLightZn/g_mLightProj[1][1];
	...
	float sum_depth = 0;
	{
		...
		for( ;light_per_row < 6; light_per_row += 2 )
		{
			...
			sum_depth =  fLightZn + max( 0,( sum_ex - fPartLit * pixel_linear_z )/( 1 - fPartLit ))*(fLightZf-fLightZn);
		}
		if( unocc_part == (light_per_row * light_per_row) )
			return float4(0,1,0,1);
		...
		//makes it farther
		//A second thought, you may want to check the Cheveshev Inequality Proof for the root of this bias
		[branch]if( sum_depth >= pixel_linear_z * (fLightZf-fLightZn) + fLightZn)
			return float4(1,0,0,1);		
	}
	
	Zmin = sum_depth;

	T_LightWidth  = ( w - Zmin ) * ( fFilterSize  ) / w,
	T_LightHeight = ( w - Zmin ) * ( fFilterSize ) / w,

	S_LightWidth  = fLightZn * T_LightWidth  / Zmin,
	S_LightHeight = fLightZn * T_LightHeight / Zmin,
	S_LightWidthNorm  = S_LightWidth  / NpWidth,
	S_LightHeightNorm = S_LightHeight / NpHeight;
